...
func (h *JWTBearerAuthenticationHandler) AuthType() string {
	return SchemeJwtBearer
}

// Authenticate 实现 AuthenticationHandler 接口
func (h *JWTBearerAuthenticationHandler) Authenticate(r *http.Request) (*web.ClaimsPrincipal, error) {
	// 先拿 token
...
func (h *JWTBearerAuthenticationHandler) validateToken(tokenString string) (*web.ClaimsPrincipal, error) {
	...
	// 构建 ClaimsPrincipal
	principal := &web.ClaimsPrincipal{
		Claims: make([]web.Claim, 0, len(claims)),
	}
